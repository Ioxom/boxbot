...
public final class MainListener extends ListenerAdapter {
    ...
    public void onMessageReceived(@NotNull final MessageReceivedEvent event) {
        ...
        if (messageContentRaw.startsWith(prefix) && !event.getAuthor().isBot()) {
            ...
            //TODO: I'm unsure whether it's faster to do this check or leave it out
            boolean isCommand = false;
            for (final String[] commandList : COMMANDS) {
                if (arrayContains(commandList, messageContent[0])) {
                    isCommand = true;
                    break;
                }
            }
